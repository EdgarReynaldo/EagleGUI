      ...
      while (!sys->UpToDate()) {
         ...
         while (gui.HasMessages()) {
            ...
            if (msg == WidgetMsg(foptions.MItems()[0] , TOPIC_MENU , MENU_ITEM_ACTIVATED)) {
               ...
               ALLEGRO_FILECHOOSER* fchoose = al_create_native_file_dialog("./" , "Pick a file to load" , "*.*" , ALLEGRO_FILECHOOSER_FILE_MUST_EXIST);
               al_show_native_file_dialog(dynamic_cast<Allegro5GraphicsContext*>(win)->AllegroDisplay() , fchoose);
               std::vector<std::string> filenames;
               for (unsigned int i = 0 ; i < al_get_native_file_dialog_count(fchoose) ; ++i) {
